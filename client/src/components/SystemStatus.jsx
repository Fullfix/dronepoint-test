...
const SystemStatus = () => {
    ...
    const { connection: { drone, dronepoint} } = useContext(DronepointContext);
    ...
    return (
        <Grid container className={classes.root} justify="space-evenly">
            ...
            <Grid item className={classes.statusBox}>
                <Typography variant="h2" className={classes.text}>Dronepoint</Typography>
                {getStatusIcon(dronepoint)}
                <Typography variant="h3">{getStatusText(dronepoint)}</Typography>
            </Grid>
        </Grid>
    )
}
